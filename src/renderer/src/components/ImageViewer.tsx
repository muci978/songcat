...
export function ImageViewer({
  ...
  const handleDoubleClick = useCallback(() => {
    if (!fs) return
    if (scale === 1) { applyScale(2); setOffset({ x: 0, y: 0 }) }
    else { resetView() }
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(num) && num >= Math.round(MIN_SCALE * 100) && num <= Math.round(MAX_SCALE * 100)) {
      setScale(num / 100); setOffset({ x: 0, y: 0 })
    }
  }

  const handleInputBlur = () => {
    const num = parseInt(inputVal, 10)
    if (isNaN(num) || num < Math.round(MIN_SCALE * 100)) applyScale(MIN_SCALE)
    else if (num > Math.round(MAX_SCALE * 100)) applyScale(MAX_SCALE)
    else { applyScale(num / 100); setOffset((p) => (p.x !== 0 || p.y !== 0) ? { x: 0, y: 0 } : p) }
  }
